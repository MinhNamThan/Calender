...
public class AlarmEvent {
    ...
    public static void alarmDay(){
        for(int t = 0; t<4;t++) {
            String sql = "select * from "+ kindEvent[t] +" where dateStart = '" + LocalDate.now().plusDays(1).toString() + "'and reminder = N'Trước 1 ngày'";
            try {
    ...
    public static void alarmMonth(){
        for(int t = 0; t<4;t++) {
            String sql = "select * from "+ kindEvent[t] +" where dateStart = '" + LocalDate.now().plusMonths(1).toString() + "'and reminder = N'Trước 1 tháng'";
            try {
                pstST = conST.prepareStatement(sql);
                rsST = pstST.executeQuery();
                while (rsST.next()) {
    ...
    public static void getAlarmMinues(){
        ...
        //timecur[1] = String.valueOf(Integer.parseInt(timecur[1]) + 1);

        for(int t = 0; t<4;t++) {
            for(int j = 0; j<3; j++) {
                String sql = "select * from " + kindEvent[t] + " where dateStart = '" + LocalDate.now().toString() + "'and reminder = N'"+reminder[j]+"' and CAST(LEFT(timeStart,CHARINDEX(':',timeStart,1) -1) AS INT) >=" + timecur[0]+ " and CAST(RIGHT(timeStart,LEN(timeStart) - CHARINDEX(':',timeStart,1)) AS INT) >=" + timecur[1];
                try {
                    pstST = conST.prepareStatement(sql);
                    rsST = pstST.executeQuery();
                    while (rsST.next()) {
                ...
                for(int u = 0; u < 4; u++){
                    sql = "select * from "+ kindEvent[t] +" where repeated = N'"+repeatName[t]+"'and reminder = N'"+reminder[j]+"' and CAST(LEFT(timeStart,CHARINDEX(':',timeStart,1) -1) AS INT) >=" + timecur[0]+ " and CAST(RIGHT(timeStart,LEN(timeStart) - CHARINDEX(':',timeStart,1)) AS INT) >=" + timecur[1];
                    try {
                        pstST = conST.prepareStatement(sql);
                        rsST = pstST.executeQuery();
                        while (rsST.next()) {
    ...
    public static void alertInfo(){
        ...
        if(time.getCurrentTime().equals(info[dem].getStartTime()+":0")){
            AlertDialog.display("Thông báo","Bạn sắp có 1 "+ info[dem].getKindEvent()+" : " + info[dem].getTitle());
            dem = dem + 1;
        }
        if(dem == sz) return;
    ...
    public static void deleteOldEvent(){
        ...
        for(int t = 0; t<4;t++) {
            String sql = "Delete from " + kindEvent[t] + " where dateFinish <= '" + LocalDate.now().minusMonths(1).toString() + "' and repeated = N'Không lặp lại'";
            try {
                pstST = conST.prepareStatement(sql);
